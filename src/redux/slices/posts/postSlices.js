...
export const createpostAction = createAsyncThunk(
  ...
  async (post, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axiosInterceptors.post(`/api/posts`, formData);
      //dispatch action
      dispatch(resetPost());
      return data;
    } catch (error) {
...
export const deletePostAction = createAsyncThunk(
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    try {
      //http call
      const { data } = await axiosInterceptors.delete(`/api/posts/${postId}`);
      //dispatch
...
export const fetchPostsAction = createAsyncThunk(
  ...
  async (category, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axiosInterceptors.get(
...
export const fetchPostDetailsAction = createAsyncThunk(
  ...
  async (id, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axiosInterceptors.get(`/api/posts/${id}`);
      return data;
    } catch (error) {
      if (!error?.response) throw error;
      return rejectWithValue(error?.response?.data);
...
export const toggleAddLikesToPost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axiosInterceptors.put(`/api/posts/likes`, {
        postId,
      });
...
export const toggleAddDisLikesToPost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axiosInterceptors.put(`/api/posts/dislikes`, {
        postId,
      });
